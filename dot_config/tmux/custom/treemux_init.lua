				...
				commands = {
					...
					show_preview = function(state)
						...
						local cmd = string.format(
							"%s %s | less -R",
							vim.fn.exepath("fzf-preview"),
							vim.fn.shellescape(node.path)
						)
						vim.fn.jobstart({
							"tmux",
							"popup",
					...
					autojump = function()
						local _z =
							":reload:zoxide query {q} -l --exclude '${PWD}' | awk '{ if (!seen[tolower()]++) print }' || true"
						vim.fn.jobstart(
							{
								"fzf",
								"--bind",
					...
					open_in_tmux = function(state, target)
						...
						local result =
							vim.fn.system('tmux display -p -t "' .. pane .. '" "#{pane_current_command} #{pane_pid}"')
						vim.fn.jobstart({ "tmux", "selectp", "-t", pane })
                ...
                nvr --serverlist | grep $pid ]]
							...
							elseif proc == "zsh" then
								...
								vim.fn.jobstart({
									...
									"splitw",
									"-v",
									"nvim " .. vim.fn.shellescape(node.path),
									"Enter",
								})
							end
						else
						...
						["!"] = {
							function(state)
								...
								vim.api.nvim_feedkeys(
									vim.api.nvim_replace_termcodes("<Home>", true, true, true),
									"n",
									true
								)
