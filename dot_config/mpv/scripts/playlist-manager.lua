...
--END OF SAMPLE REPLACE ]=====]
  ...
  style_ass_tags = "{\\fn.SFNS-Regular\\fs10\\b0\\bord0.5\\c&HF3F3F3&}",
  ...
  normal_file = "○  %name",
  hovered_file = "●  %name",
  selected_file = "{\\fn.SFNS-Regular}➔{\\fn.SFNS-Regular}  %name",
  playing_file = "▷  %name",
  playing_hovered_file = "▶  %name",
  playing_selected_file = "{\\fn.SFNS-Regular}➤{\\fn.SFNS-Regular}  %name",
...
function on_loaded()
  ...
  if settings.save_last_session then
    local savepath = mp.find_config_file("last-session.m3u")
...
function add_prev_part()
    refresh_globals()
    local path_url
    local index
    local params = ''
    if playlist_visible then
        path_url = mp.get_property("playlist/"..cursor.."/filename")
    else
        path_url = mp.get_property("path")
    end
    if string.match(path_url, "^https://www.bilibili.com/") then
        local before = string.match(path_url, "^https://www.bilibili.com/.*([?&]p=%d+)")
        if before == nil then
            local bvid = string.match(path_url, "^https://www.bilibili.com/video/(%w+)")
            mp.command_native_async(
            {
                name='subprocess', 
                capture_stdout=true, 
                playback_only=false,
                args={"curl", "-sL", "https://api.bilibili.com/x/web-interface/view?bvid="..bvid}
            }, function(s, r, e)
                if e then 
                    mp.osd_message("Error: "..e)
                    return
                end
                local data = utils.parse_json(r.stdout:gsub("\n+", "")).data
                if data.season_id == nil then return end
                local new_url
                for _, s in pairs(data.ugc_season.sections) do
                    for i, e in pairs(s.episodes) do
                        if e.bvid == bvid then
                            if s.episodes[i - 1] == nil then return end
                            bvid = s.episodes[i - 1].bvid
                            new_url = "https://bilibili.com/video/"..bvid
                            break
                        end
                    end
                    if new_url ~= nil then break end
                end
                if new_url == nil then return end
                mp.commandv("loadfile", new_url, "append")
                if playlist_visible then
                    showplaylist()
                else
                    mp.osd_message("Added to playlist: " .. new_url)
                end
            end)
        else
            if tonumber(string.sub(before, 4)) == 1 then return end
            local after = string.match(before, ".*=") .. string.match(before, "%d+") - 1
            path_url = string.gsub(path_url, before, after)
            mp.commandv("loadfile", path_url, "append")
            if playlist_visible then
                showplaylist()
            else
                mp.osd_message("Added to playlist: " .. path_url)
            end
        end
        return
    elseif string.match(path_url, "^https://www.youtube.com/.*[?&]list=%w+") and string.match(path_url, "[?&]index=%d+") then
        index = string.match(path_url, "[?&]index=(%d+)") - 1
        if index == 0 then return end
        params = string.format("&list=%s&index=%d", string.match(path_url, "[?&]list=(%w+)"), index)
    else
        return
    end
    mp.command_native_async(
    {
        ...
        capture_stdout=true, 
        playback_only=false,
        args={"yt-dlp", path_url, "-I", tostring(index), "-O", "webpage_url"}
    }, function(s, r, e)
        if e then 
            mp.osd_message("Error: "..e)
            return
        end
        path_url = r.stdout:gsub("%s+", "")
        if path_url == '' then return end
        mp.commandv("loadfile", path_url..params, "append")
        if playlist_visible then
            showplaylist()
        else
            mp.osd_message("Added to playlist: " .. path_url..params)
        end
    end)
end

function add_next_part(rest_of_playlist)
    refresh_globals()
    local path_url
    local index
    local params = ''
    ...
    if string.match(path_url, "^https://www.youtube.com/.*[?&]list=%w+") and string.match(path_url, "[?&]index=%d+") then
        index = string.match(path_url, "[?&]index=(%d+)") + 1
        params = string.format("&list=%s&index=%d", string.match(path_url, "[?&]list=(%w+)"), index)
    elseif string.match(path_url, "^https://www.bilibili.com/.*[?&]p=%d+") then
        index = string.match(path_url, "^https://www.bilibili.com/.*[?&]p=(%d+)") + 1
        path_url = string.gsub(path_url, "[?&]p=%d+", "")
    elseif string.match(path_url, "https://www.bilibili.com/") then
        local bvid = string.match(path_url, "^https://www.bilibili.com/video/(%w+)")
        mp.command_native_async({name='subprocess', capture_stdout=true, args={"curl", "-sL", "https://api.bilibili.com/x/web-interface/view?bvid="..bvid}}, function(s, r, e)
            if e then 
                mp.osd_message("Error: "..e)
                return
            end
            ...
            if data.season_id == nil then 
                if data.videos > 1 then
                    new_url = path_url.."?p=2"
                end
            else
                for _, s in pairs(data.ugc_season.sections) do
                    for i, e in pairs(s.episodes) do
    ...
    local episodes = tostring(index)
    if rest_of_playlist then episodes = episodes.."-" end
    mp.command_native_async({name='subprocess', capture_stdout=true, args={"yt-dlp", path_url, "-I", episodes, "-O", "webpage_url"}}, function(s, r, e)
        if e then 
            msg.error(e)
            mp.osd_message("Error: "..e)
            return
        end
        if r.stdout:gsub("%s+", "") == "" then return end
        for _, path_url in pairs(mysplit(r.stdout, "\n")) do
            msg.info(path_url)
            path_url = path_url:gsub(" ", "")
            if path_url ~= "" then
                mp.commandv("loadfile", path_url..params, "append")
                if playlist_visible then
                    showplaylist()
...
function copy_path()
    ...
    if playlist_visible then
        ...
        mp.osd_message(string.format("Path Copied: %s", path_url))
    end
end
...
function save_playlist(force_path)
  local length = mp.get_property_number('playlist-count', 0)
  if length == 0 then return end

  --get playlist save path
  local savepath = force_path
  if not savepath or savepath == "" then
    ...
    local filename = "Playlist ("..length..os.date("x) %b %d %Y, %H.%M.%S.m3u")
    savepath = utils.join_path(savepath, filename)
  end
...
function restore_last_session()
  local savepath = mp.find_config_file("last-session.m3u")
  if utils.file_info(savepath) then
    os.execute("open '"..savepath.."'")
    mp.osd_message("Loaded playlist from last session")
  end
end
...
function add_keybinds()
  bind_keys(settings.key_moveup, 'move-up', moveup, "repeatable")
  bind_keys(settings.key_movedown, 'move-down', movedown, "repeatable")
  bind_keys(settings.key_selectfile, 'select-file', selectfile)
  bind_keys(settings.key_unselectfile, 'unselect-file', unselectfile)
  bind_keys(settings.key_playfile, 'play-file', playfile)
  bind_keys(settings.key_removefile, 'remove-file', removefile, "repeatable")
  bind_keys(settings.key_closeplaylist, 'close-playlist', remove_keybinds)
end
...
function remove_keybinds()
  ...
  if settings.dynamic_binds then
    unbind_keys(settings.key_moveup, 'move-up')
    unbind_keys(settings.key_movedown, 'move-down')
    unbind_keys(settings.key_selectfile, 'select-file')
    unbind_keys(settings.key_unselectfile, 'unselect-file')
    unbind_keys(settings.key_playfile, 'play-file')
    unbind_keys(settings.key_removefile, 'remove-file')
    unbind_keys(settings.key_closeplaylist, 'close-playlist')
  end
end
...
mp.add_key_binding("CTRL+p", "sort-playlist", sortplaylist)
mp.add_key_binding("CTRL+P", "shuffle-playlist", shuffleplaylist)
mp.add_key_binding("CTRL+R", "reverse-playlist", reverseplaylist)
mp.add_key_binding("P", "load-files", playlist)
mp.add_key_binding("p", "save-playlist", save_playlist)
mp.add_forced_key_binding("p", "save-playlist", save_playlist)
mp.add_key_binding("SHIFT+ENTER", "show-playlist", toggle_playlist)
mp.add_key_binding("[", 'add-prev-part', add_prev_part)
mp.add_key_binding("]", 'add-next-part', add_next_part)
mp.add_key_binding("CTRL+]", 'add-next-parts', function() add_next_part(true) end)
mp.add_key_binding("Ctrl+c", "copy-path", copy_path)
if settings.save_last_session then
  mp.add_key_binding("Ctrl+Shift+r", "restore-last-session", restore_last_session)
end
