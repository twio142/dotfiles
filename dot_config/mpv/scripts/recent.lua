...
function unbind()
    ...
    mp.remove_key_binding("recent-COPY")
    mp.set_osd_ass(0, 0, "")
    list_drawn = false
    if auto_close_timer ~= nil then
...
function display_list()
    ...
    mp.add_forced_key_binding("Y", "recent-COPY", function()
        if not list[#list-start-choice] then return end
        set_clipboard(list[#list-start-choice].path)
    end)
    ...
    auto_close_timer = mp.add_periodic_timer(1, function()
        if mp.get_time() - timer >= 5 then unbind() end
    end)
end
...
if o.auto_save then
    ...
    mp.add_hook("on_unload", 9, function ()
        local pos = mp.get_property("percent-pos") or 0
